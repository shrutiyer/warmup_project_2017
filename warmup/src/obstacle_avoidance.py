...
class ObstacleAvoiding(object):
    ...
    def attractive_forces(self):
        goal_distance = (self.goal_odom[0]-self.curr_pos_odom[0])**2
                        +(self.goal_odom[0]-self.curr_pos_odom[0])**2
        self.goal_bl = self.odom_to_base_link(self.goal_odom)
        goal_angle = math.atan(self.goal_bl[1]/float(self.goal_bl[0]))
        angular = goal_angle/(2*math.pi)

        if goal_radius<=goal_distance:
            linear = 1
        elif goal_distance<goal_radius
            linear = goal_distance/goal_radius

        return (linear,angular)

    def odom_to_base_link(self, odom_point):
        # Receives a tuple of odom
        return (odom_point[0]*math.cos(self.curr_angle) + odom_point[1]*math.sin(self.curr_angle),
                -odom_point[0]*math.sin(self.curr_angle) + odom_point[1]*math.cos(self.curr_angle))
